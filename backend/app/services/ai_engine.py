...
# AI 응답이 완전히 비어있을 때 (스트리밍 + non-streaming retry 모두 실패) 에러 코드/메시지
# canned fallback 텍스트 저장 대신 error 이벤트로 솔직히 알려 지표 왜곡 방지
AI_EMPTY_RESPONSE_MESSAGE = "AI 응답을 받지 못했습니다. 다시 시도해주세요."
AI_EMPTY_RESPONSE_CODE = "AI_EMPTY_RESPONSE"
        ...
        if not tHasTextContent:
            ...
            try:
                ...
                if tRetryText.strip():
                    ...
                    # 재시도도 빈 응답 — canned text 대신 error 이벤트로 솔직히 알림
                    logger.warning("Non-streaming retry returned empty text for session")
                    yield {
                        "type": EVENT_TYPE_ERROR,
                        "data": {
                            "message": AI_EMPTY_RESPONSE_MESSAGE,
                            "code": AI_EMPTY_RESPONSE_CODE,
                        },
                    }
                    return
            except Exception as tRetryError:
                # 재시도 예외 — canned fallback 대신 error 이벤트로 지표 왜곡 방지
                logger.exception("Streaming retry failed for session")
                yield {
                    "type": EVENT_TYPE_ERROR,
                    "data": {
                        "message": AI_EMPTY_RESPONSE_MESSAGE,
