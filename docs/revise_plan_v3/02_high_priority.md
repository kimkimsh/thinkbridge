...
### 위치
`backend/app/services/ai_engine.py:42-52` (DEFAULT_ANALYSIS 정의) + 사용처 L247-249, **L461-464 (스트리밍 JSON decode 실패 시)**, L472-480 + `backend/app/models/thought_analysis.py`

**검증 확인** (2026-04-12): 테이블명은 실제 `thought_analyses` (복수형). 아래 SQL에서 이 이름 사용 필수.
...
# Supabase는 ALTER TABLE 실행 필요 (테이블명 주의: thought_analyses — 복수형)
```

```sql
-- Supabase SQL Editor에서 실행
ALTER TABLE thought_analyses ADD COLUMN m_is_fallback BOOLEAN DEFAULT FALSE NOT NULL;
ALTER TABLE thought_analyses ADD COLUMN m_fallback_reason VARCHAR(50);
```
...
## P1-5: `processTurn` Non-Streaming Dead Code 정리 (격하)

> **검증 결과 (2026-04-12)**: Plan 초안의 "`raise None` 가능" 주장은 **실제로는 불가능**. 루프가 `range(2)`이므로 최소 1회 try 실행, 성공 시 early return, 실패 시 tLastError 할당 보장 → 루프 종료 시 tLastError non-None 보장. **이 항목을 P1에서 격하하고 DOC-9 (processTurn dead code 문서화)와 통합 처리**.

### 위치
`backend/app/services/ai_engine.py:295-331`

### 실제 상태
- `processTurn`은 어떤 라우터에서도 호출되지 않음 (`routers/` grep 0 matches)
- `ai_engine.py:34`의 routers import는 `processTurnStreaming`만 사용
- 즉 **완전히 dead code**

### 권장 조치

**Option A (권장) — DOC-9와 통합, 삭제**:
`processTurn` 함수 전체 삭제. Services `__init__.py`의 export도 제거.

**Option B — Safety net 유지**:
삭제하지 않고 남기되 docstring에 "non-production path, reference only" 명시 + 만일을 대비한 catch-all 추가.

### 변경량
- A: XS (~5줄 — 함수 + export 삭제)
- B: S (~10줄 — safety net + docstring)

### 리스크
없음. 호출처 없으므로 삭제해도 동작 영향 0.

**통합 처리**: DOC-9와 함께 한 commit에서 처리. 이 항목은 **P1에서 제외하고 P2-extra 또는 Docs 카테고리로 이동 고려**.
...
> **검증 확인 (2026-04-12)**: Plan 초안이 "`asyncio.CancelledError`를 에러로 분류"라고 주장했으나 **부정확**. Python 3.8+에서 `asyncio.CancelledError`는 `BaseException` 직계 상속이므로 `except Exception`에 **포착되지 않음**. 현재 코드는 cancel을 정상 전파하고 있음.

1. `logger.error` → traceback 안 찍힘. Root cause 파악 불가 (`logger.exception`이 필요)
2. 모든 에러가 같은 generic 메시지 → 운영자 진단 어려움
3. `CancelledError` 명시적 처리를 추가하면 "의도 표현"으로 유지보수성 향상. 현재는 암묵적 정상 동작 — 나중에 `except BaseException` 추가 시 safety 필요.
4. `APIError` 등 세분된 타입도 구분 없이 묶임 → 로깅 개선 여지

### Fix Approach

**의도 표현용** 명시적 `CancelledError` 처리 (현재는 암묵적 전파 — 명시화가 유지보수성 향상):

```python
except asyncio.CancelledError:
    logger.info("Client disconnected during streaming for session %d", tSessionId)
    raise  # explicitly propagate cancellation (Python 3.8+ BaseException subclass, redundant but clear)
...
const handleGuestTrial = async () => {
    setIsGuestLoading(true);
    setError(null);  // 기존 mError state 재사용 (login/page.tsx L50에 이미 존재)
    try {
        await loginAsGuest();
        router.push("/student/chat");
    } catch (tError) {
...
// 렌더링 부분에 error banner 추가 (ChatInterface.tsx L530-550 기존 inline banner 패턴과 동일)
{mError && (
...
### 리스크
- 에러 UI가 hero 섹션과 시각적 충돌 가능 — **inline banner 패턴 권장** (ChatInterface.tsx L530-550의 기존 패턴과 일관). ~~Sonner~~ 프로젝트에 **미설치** 확인 (grep 결과 0 matches) — Sonner 언급 제거.
